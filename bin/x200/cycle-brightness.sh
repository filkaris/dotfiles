...
if [ $BRIGHTNESS -ge 15 ]; then
    $HOME/.dotfiles/bin/x200/set-brightness.sh 11
    exit 0;
fi;

if [ $BRIGHTNESS -ge 11 ]; then
    $HOME/.dotfiles/bin/x200/set-brightness.sh 7
    exit 0;
fi;

if [ $BRIGHTNESS -ge 7 ]; then
    $HOME/.dotfiles/bin/x200/set-brightness.sh 3
    exit 0;
fi;

if [ $BRIGHTNESS -ge 3 ]; then
    $HOME/.dotfiles/bin/x200/set-brightness.sh 15
    exit 0;
fi;
